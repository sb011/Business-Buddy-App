...
class InventoryAPI {
  ...
  static Future<List<Item>> getInventoryItems({
    ...
    final uri = Uri.parse('$_baseUrl/${ApiEndpoints.inventoryItems}').replace(queryParameters: qp);
    ...
    if (response.statusCode != 200 && response.statusCode != 201) {
      try {
        ...
        final message = resp['errorMessage']?.toString() ?? 'Failed to fetch inventory items';
        throw Exception(message);
      } catch (_) {
        throw Exception('Failed to fetch inventory items. Status: ${response.statusCode}');
      }
    } else {
